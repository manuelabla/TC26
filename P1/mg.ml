...
let check s =                                                               (*Función de checkeo de strings en el teorema mg*)
  ...
  match l with 
  |[x; y; z] -> if String.length x >= 0                                     (*---CONDICIONES---*)
                ...
                then Printf.printf "yes" else Printf.printf "no"            (*Si se cumple todo -> yes || No se cumple -> no*)
  | _ -> Printf.printf "no"                                                 (*Resto de casos -> no*)
